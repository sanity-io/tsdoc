import {APIClass, APIEnum, APIInterface, APIMember, APINamespace, Sanity} from '@sanity/tsdoc'
import {APIMemberWithInheritance} from './_types'

export function _getMembers(data: APIMember): Sanity.ArrayItem<APIMemberWithInheritance>[] {
  if (!('members' in data)) {
    return []
  }

  let mems: Sanity.ArrayItem<
    | APIClass['members'][0]
    | APIEnum['members'][0]
        ...
        if (member && 'members' in member) {
          mems = mems.concat(
            member.members.map((mem) => {
              const m: Sanity.ArrayItem<APIMemberWithInheritance> = {
                ...mem,
