import {studioTheme, ThemeFonts} from '@sanity/ui'

export const fonts: ThemeFonts = {
  code: {
    family: studioTheme.fonts.code.family,
    // family: '-apple-system-ui-monospace, "SF Mono", Menlo, Monaco, Consolas, monospace',
    weights: {
      regular: 500,
      medium: 600,
      semibold: 700,
      bold: 800,
    },
    sizes: [
      // Code 0
      {
        ascenderHeight: 5,
        descenderHeight: 5,
        fontSize: 12,
        iconSize: 17,
        lineHeight: 17,
        letterSpacing: 0,
      },
      // Code 1
      {
        ascenderHeight: 6,
        descenderHeight: 6,
        fontSize: 14,
        iconSize: 19,
        lineHeight: 21,
        letterSpacing: 0,
      },
      // Code 2
      {
        ascenderHeight: 7,
        descenderHeight: 7,
        fontSize: 16,
        iconSize: 23,
        lineHeight: 25,
        letterSpacing: 0,
      },
      // Code 3
      {
        ascenderHeight: 8,
        descenderHeight: 8,
        fontSize: 18,
        iconSize: 29,
        lineHeight: 29,
        letterSpacing: 0,
      },
      // Code 4
      {
        ascenderHeight: 9,
        descenderHeight: 9,
        fontSize: 21,
        iconSize: 33,
        lineHeight: 33,
        letterSpacing: 0,
      },
    ],
  },
  heading: {
    family: `Larsseit, ${studioTheme.fonts.heading.family}`,
    // family:
    //   'Larsseit, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", sans-serif',
    weights: {
      regular: 700,
      medium: 800,
      semibold: 900,
      bold: 900,
    },
    sizes: [
      // Heading 1
      {
        ascenderHeight: 3,
        descenderHeight: 3,
        fontSize: 14,
        iconSize: 23,
        lineHeight: 21,
        letterSpacing: 0,
      },
      // Heading 2
      {
        ascenderHeight: 4,
        descenderHeight: 4,
        fontSize: 16,
        iconSize: 25,
        lineHeight: 23,
        letterSpacing: 0,
      },
      // Heading 0
      {
        ascenderHeight: 3,
        descenderHeight: 3,
        fontSize: 18,
        iconSize: 17,
        lineHeight: 25,
        letterSpacing: 0,
      },
      // Heading 1
      {
        ascenderHeight: 5,
        descenderHeight: 5,
        fontSize: 24,
        iconSize: 33,
        lineHeight: 33,
        letterSpacing: 0,
      },
      // Heading 2
      {
        ascenderHeight: 6,
        descenderHeight: 6,
        fontSize: 33,
        iconSize: 41,
        lineHeight: 41,
        letterSpacing: 0,
      },
      // Heading 3
      {
        ascenderHeight: 7,
        descenderHeight: 7,
        fontSize: 42,
        iconSize: 49,
        lineHeight: 49,
        letterSpacing: 0,
      },
      // Heading 4
      {
        ascenderHeight: 11,
        descenderHeight: 11,
        fontSize: 49,
        iconSize: 57,
        lineHeight: 57,
        letterSpacing: 0,
      },
      // Heading 5
      {
        ascenderHeight: 15,
        descenderHeight: 15,
        fontSize: 59,
        iconSize: 65,
        lineHeight: 65,
        letterSpacing: 0,
      },
    ],
  },
  label: {
    family: studioTheme.fonts.code.family,
    // family:
    //   'Roboto Mono, Inter var, system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", sans-serif',
    weights: {
      regular: 600,
      medium: 700,
      semibold: 800,
      bold: 900,
    },
    sizes: [
      // Label 0
      {
        ascenderHeight: 2,
        descenderHeight: 2,
        fontSize: 11,
        iconSize: 19,
        lineHeight: 12,
        letterSpacing: 0.5,
      },
      // Label 1
      {
        ascenderHeight: 3,
        descenderHeight: 3,
        fontSize: 13,
        iconSize: 21,
        lineHeight: 15,
        letterSpacing: 0.5,
      },
      // Label 2
      {
        ascenderHeight: 4,
        descenderHeight: 3,
        fontSize: 14,
        iconSize: 23,
        lineHeight: 17,
        letterSpacing: 0.5,
      },
      // Label 3
      {
        ascenderHeight: 5,
        descenderHeight: 4,
        fontSize: 16,
        iconSize: 25,
        lineHeight: 19,
        letterSpacing: 0.5,
      },
    ],
  },
  text: {
    family: studioTheme.fonts.text.family,
    weights: {
      regular: 500,
      medium: 600,
      semibold: 700,
      bold: 800,
    },
    sizes: [
      // Text 0
      {
        ascenderHeight: 5,
        descenderHeight: 5,
        fontSize: 12,
        iconSize: 17,
        lineHeight: 17,
        letterSpacing: 0,
      },
      // Text 1
      {
        ascenderHeight: 6,
        descenderHeight: 6,
        fontSize: 14,
        iconSize: 19,
        lineHeight: 21,
        letterSpacing: 0,
      },
      // Text 2
      {
        ascenderHeight: 7,
        descenderHeight: 7,
        fontSize: 16,
        iconSize: 23,
        lineHeight: 25,
        letterSpacing: 0,
      },
      // Text 3
      {
        ascenderHeight: 8,
        descenderHeight: 8,
        fontSize: 18,
        iconSize: 29,
        lineHeight: 29,
        letterSpacing: 0,
      },
      // Text 4
      {
        ascenderHeight: 9,
        descenderHeight: 9,
        fontSize: 21,
        iconSize: 33,
        lineHeight: 33,
        letterSpacing: 0,
      },
    ],
  },
}
